...
function startEventsObserver(eventTypes: PerformanceEvent[]) {
  eventTypes.forEach((type) => {
    // Start Observations
    const observer = new PerformanceObserver((list) =>
      list.getEntries().forEach((entry: any) => {
        const entryType = entry.entryType as PerformanceEvent;
...
function startManualEventsObserver(eventTypes: PerformanceEvent[]) {
...
function getPerformanceData() {
  const entryTypes: PerformanceEvent[] = [
    "resource",
...
// * Main
const newData: GoldEventMessage = {
  clickEvents: [],
  keyEvents: [],
  mouseMoveEvents: [],
  tabVisibilityEvents: [],
