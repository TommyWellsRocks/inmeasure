...
export function TotalVisitorsTable({
  ...
  connectionTimestamps.forEach((timestamp) => {
    ...
    // If in the last minute, activeUsere
    if (howLongAgo <= timeToMS(1, "minutes")) {
      currentActiveUsers++;
      prevHourUsers++;
      prev24HourUsers++;
      prev7DayUsers++;
    ...
    // In the last 5 years
    else if (howLongAgo <= timeToMS(5, "years")) {
      prev5YearUsers++;
    }
  });
  ...
  const items = [
    ...
    { col1: "Last 5 Years", col2: prev5YearUsers },
  ];
